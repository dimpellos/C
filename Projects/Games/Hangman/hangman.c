...
void
display(const Game* game, const char* guess)
{
  int wg = game->wg;
...
readwords(const char* fn)
{
  ...
  for (char* tok = strtok(bp, "\n"); tok; tok = strtok(NULL, "\n")) {
    len = strlen(tok);
    if (len < 3 || len > 50)
      continue; // longest word in major dict  has 45 words, rest are chemicals
    for (char* p = tok; *p; ++p) {
      if (isalpha(*p))
        *p = tolower(*p);
      else
  ...
  free(bp);
  return (
    Buffer){ .words = ilen / sizeof ilen, .index = (size_t*)wi, .buffer = wl };
}
int
shuffle(Buffer* buf)
{
...
main(void)
{
  ...
  do {
    ...
    } while (strcmp(unknown, guess) && ch != ('d' & 0x1f) && ch != EOF &&
             game.wg < 6);
